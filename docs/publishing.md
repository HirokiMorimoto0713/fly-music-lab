...
`scripts/build-pages.mjs` が、4画面のHTML、Git管理された `src/` と `docs/` の対象ファイル、README、全配線データ、LIF実装、必要なThree.jsの2モジュールとライセンスを選びます。データは `provenance.json` のSHA-256と一致することを確認します。
...
1. `npm ci --ignore-scripts`、`npm run prepare:data`、`npm test` を実行します。
